...
class SnopesFactCheckingSiteExtractor(FactCheckingSiteExtractor):
    ...
    #la fonction permet de compter le nbr de page du site de snopes
    def find_page_count(self, parsed_listing_page: BeautifulSoup) -> int:
        #print(parsed_listing_page)
        next_link = parsed_listing_page.find("a", {"class", "page-link font-weight-bold"})['href']#changer la classe
        ...
        print(max_page)
        return max_page
    # la fonction recupére les lien des claims dans tous les pages
    def retrieve_urls(self, parsed_listing_page: BeautifulSoup, listing_page_url: str, number_of_pages: int) \
        ...
        for page_number in tqdm(range(2, number_of_pages)):
            ...
            #print(self.extract_urls(current_parsed_listing_page))
        return urls
    ...
    def extract_claim_and_review(self, parsed_claim_review_page: BeautifulSoup, url: str) -> List[Claim]:
        ...
        rating = None
        claim_text = None
        date1=parsed_claim_review_page.find("footer")
        date2=date1.find("li",{"class": "font-weight-bold textr-muted"})
        #print(date2)
        date_ = date2
        print(date_)
        if not date_:
        ...
        # author
        author = parsed_claim_review_page.find("a", {"class": "font-weight-bold"})
        #print(author)
        ...
        if not rating:
            rating_div = parsed_claim_review_page.find("div", {"class": "claim-text card-body"})
        ...
        if not rating:
            # Oldest page format
            rating = parsed_claim_review_page.find("font", {"class", "claim-text card-body"})
            print(rating)
            if rating:
                rating = rating.find("b")"""
        
					
        rating1 = parsed_claim_review_page.find("div", {"class", "media-body d-flex flex-column align-self-center"})
