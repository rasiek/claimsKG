...
class SnopesFactCheckingSiteExtractor(FactCheckingSiteExtractor):
    ...
    def find_page_count(self, parsed_listing_page: BeautifulSoup) -> int:
        next_link = parsed_listing_page.find("a", {"class", "page-link font-weight-bold"})['href'] # modification du next_link
    ...
    def retrieve_urls(self, parsed_listing_page: BeautifulSoup, listing_page_url: str, number_of_pages: int) \
        ...
        for page_number in tqdm(range(2, number_of_pages)):
            if 0 < self.configuration.maxClaims < len(urls):
                break
            url = listing_page_url + "page/" + str(page_number) # modification de /page/ en page/
    ...
    def extract_urls(self, parsed_listing_page: BeautifulSoup):
        urls = list()
        links = parsed_listing_page.find("div",{"class":"list-group list-group-flush"}) # modification  
        links = links.findAll('a')
        for anchor in links:
            # enlèvement de anchor = anchor.find("a",href=true)
            url = str(anchor['href'])
            # print(url)
    ...
    def extract_claim_and_review(self, parsed_claim_review_page: BeautifulSoup, url: str) -> List[Claim]:
        ...
        # title
        article = parsed_claim_review_page.find("article") # modification
        if article is None:
        ...
        # enlèvement de card = article.find("div", {"class": "content-wrapper card"})
        card_body = article.find("div",{"class":"single-body card card-body rich-text"}) # modification

        # date
        date_str = ""
        rating = None
        claim_text = None
        date_ = parsed_claim_review_page.find('ul', {"class": "list-unstyled dates list-unstyled d-flex flex-wrap comma-separated-md"})
        # print date_["content"]
        if len(date_.findAll("li")) > 1: 
            date_ = date_.findAll("li")[1] # date de mise à jour si jamais il y en a une
        if len(date_.findAll("li")) == 1: 
            date_ = date_.findAll("li")[0] # date de publication
        if date_: # modification
            date_ = re.search("\d+\ .+\ \d+",date_.text).group(0)
            if date_:
                date_str = dateparser.parse(date_).strftime("%Y-%m-%d")
        ...
        # author
        author = parsed_claim_review_page.find("ul", {"class": "list-unstyled authors list-unstyled d-flex flex-wrap comma-separated"}) # modification
        author = author.find('a')

        rating_div = None
        rating1 = parsed_claim_review_page.find("div",{"class":"media-body d-flex flex-column align-self-center"}) # modification
        if rating1 is not None:
            rating = rating1.find("span")
